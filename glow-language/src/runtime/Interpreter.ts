...
export class GlowRuntimeError extends Error {
  constructor(message: string, public glowLine?: number, public glowCol?: number) {
    super(message);
...
export type ImportResolver = (path: string) => Statement[];
...
export class Interpreter {
  ...
  constructor() {
    ...
    this.globals.define("random", {
      type: "NativeFunction",
      call: (args: any[]) => {
        if (args.length === 0) return Math.floor(Math.random() * 2);
    ...
    this.globals.define("floor", {
      type: "NativeFunction",
      call: (args: any[]) => Math.floor(args[0])
    });

    this.globals.define("ceil", {
      type: "NativeFunction",
      call: (args: any[]) => Math.ceil(args[0])
    });

    this.globals.define("round", {
      type: "NativeFunction",
      call: (args: any[]) => Math.round(args[0])
    });

    this.globals.define("sqrt", {
      type: "NativeFunction",
      call: (args: any[]) => Math.sqrt(args[0])
    });

    this.globals.define("abs", {
      type: "NativeFunction",
      call: (args: any[]) => Math.abs(args[0])
    });

    this.globals.define("pow", {
      type: "NativeFunction",
      call: (args: any[]) => Math.pow(args[0], args[1])
    });

    this.globals.define("max", {
      type: "NativeFunction",
      call: (args: any[]) => Math.max(...args)
    });

    this.globals.define("min", {
      type: "NativeFunction",
      call: (args: any[]) => Math.min(...args)
    });

    this.globals.define("clamp", {
      type: "NativeFunction",
      call: (args: any[]) => Math.min(Math.max(args[0], args[1]), args[2])
    });

    this.globals.define("log", {
      type: "NativeFunction",
      call: (args: any[]) => args.length > 1 ? Math.log(args[0]) / Math.log(args[1]) : Math.log(args[0])
    });

    this.globals.define("sign", {
      type: "NativeFunction",
      call: (args: any[]) => Math.sign(args[0])
    });
    ...
    this.globals.define("isNumber", {
      type: "NativeFunction",
      call: (args: any[]) => typeof args[0] === "number"
    });

    this.globals.define("isString", {
      type: "NativeFunction",
      call: (args: any[]) => typeof args[0] === "string"
    });

    this.globals.define("isBool", {
      type: "NativeFunction",
      call: (args: any[]) => typeof args[0] === "boolean"
    });

    this.globals.define("isArray", {
      type: "NativeFunction",
      call: (args: any[]) => Array.isArray(args[0])
    });
    ...
    this.globals.define("isNull", {
      type: "NativeFunction",
      call: (args: any[]) => args[0] === null || args[0] === undefined
    });

    // ─── TYPE CONVERSION ──────────────────────────────────────────────────────
    this.globals.define("toNumber", {
      type: "NativeFunction",
      call: (args: any[]) => Number(args[0])
    });
    ...
    this.globals.define("toBool", {
      type: "NativeFunction",
      call: (args: any[]) => Boolean(args[0])
    });

    // ─── I/O ──────────────────────────────────────────────────────────────────
    this.globals.define("printLine", {
      type: "NativeFunction",
      call: (_args: any[]) => { process.stdout.write("\n"); return null; }
    });
    ...
    this.globals.define("padStart", {
      type: "NativeFunction",
      call: (args: any[]) => String(args[0]).padStart(Number(args[1]), args[2] ?? " ")
    });

    this.globals.define("padEnd", {
      type: "NativeFunction",
      call: (args: any[]) => String(args[0]).padEnd(Number(args[1]), args[2] ?? " ")
    });

    this.globals.define("strRepeat", {
      type: "NativeFunction",
      call: (args: any[]) => String(args[0]).repeat(Number(args[1]))
    });
    ...
    this.globals.define("split", {
      type: "NativeFunction",
      call: (args: any[]) => String(args[0]).split(args[1] ?? "")
    });

    this.globals.define("join", {
      type: "NativeFunction",
      call: (args: any[]) => (args[0] as any[]).join(args[1] ?? "")
    });

    this.globals.define("trim", {
      type: "NativeFunction",
      call: (args: any[]) => String(args[0]).trim()
    });

    this.globals.define("upper", {
      type: "NativeFunction",
      call: (args: any[]) => String(args[0]).toUpperCase()
    });

    this.globals.define("lower", {
      type: "NativeFunction",
      call: (args: any[]) => String(args[0]).toLowerCase()
    });

    this.globals.define("contains", {
      type: "NativeFunction",
      call: (args: any[]) => String(args[0]).includes(String(args[1]))
    });

    this.globals.define("startsWith", {
      type: "NativeFunction",
      call: (args: any[]) => String(args[0]).startsWith(String(args[1]))
    });

    this.globals.define("endsWith", {
      type: "NativeFunction",
      call: (args: any[]) => String(args[0]).endsWith(String(args[1]))
    });

    this.globals.define("replace", {
      type: "NativeFunction",
      call: (args: any[]) => String(args[0]).replaceAll(String(args[1]), String(args[2]))
    });
    ...
    this.globals.define("push", {
      type: "NativeFunction",
      call: (args: any[]) => { (args[0] as any[]).push(args[1]); return null; }
    });

    this.globals.define("pop", {
      type: "NativeFunction",
      call: (args: any[]) => (args[0] as any[]).pop() ?? null
    });

    this.globals.define("includes", {
      type: "NativeFunction",
      call: (args: any[]) => (args[0] as any[]).includes(args[1])
    });
    ...
    this.globals.define("indexOf", {
      ...
      call: (args: any[]) => {
        const v = args[0];
        if (Array.isArray(v)) return v.indexOf(args[1]);
        if (typeof v === "string") return v.indexOf(String(args[1]));
        return -1;
      }
    });
    ...
    this.globals.define("slice", {
      type: "NativeFunction",
      call: (args: any[]) => (args[0] as any).slice(args[1], args[2])
    });
    ...
    this.globals.define("sort", {
      ...
      call: (args: any[]) => {
        ...
        if (args[1] === "text") return arr.sort();
    ...
    this.globals.define("keys", {
      type: "NativeFunction",
      call: (args: any[]) => Object.keys(args[0])
    });

    this.globals.define("values", {
      type: "NativeFunction",
      call: (args: any[]) => Object.values(args[0])
    });

    this.globals.define("hasKey", {
      type: "NativeFunction",
      call: (args: any[]) => args[1] in args[0]
    });

    this.globals.define("merge", {
      type: "NativeFunction",
      call: (args: any[]) => ({ ...args[0], ...args[1] })
    });

    // ─── TIME ─────────────────────────────────────────────────────────────────
    this.globals.define("time", {
      type: "NativeFunction",
      call: (_args: any[]) => Date.now() / 1000   // seconds since epoch, like most langs
    });

    this.globals.define("timeMs", {
      type: "NativeFunction",
      call: (_args: any[]) => Date.now()
    });

    // ─── PROGRAM ──────────────────────────────────────────────────────────────
    this.globals.define("exit", {
      type: "NativeFunction",
      call: (args: any[]) => process.exit(args[0] ?? 0)
    });

    this.globals.define("error", {
      type: "NativeFunction",
      call: (args: any[]) => { throw new GlowRuntimeError(String(args[0])); }
    });
  }

  setImportResolver(resolver: ImportResolver) {
  ...
  private executeBlock(statements: Statement[], environment: Environment): any {
  ...
  private execute(statement: Statement, environment: Environment): any {
    ...
    switch (statement.type) {
      ...
      case "ImportStatement": {
        if (!this.importResolver) throw new Error("import is not available in this context");
      ...
      case "WhileStatement":
        ...
        while (this.evaluate(statement.condition, environment)) {
          if (this.timedOut) throw new Error("Execution stopped by user");
          if (iterations++ > MAX_ITERATIONS) throw new Error("Infinite loop detected");
      ...
      case "RepeatStatement":
        ...
        for (let i = 0; i < this.evaluate(statement.count, environment); i++) {
          if (this.timedOut) throw new Error("Execution stopped by user");
          if (repIterations++ > MAX_ITERATIONS) throw new Error("Infinite loop detected");
          const r = this.executeBlock(statement.body, new Environment(environment));
          if (r instanceof ReturnValue) return r;
        }
        return;
  ...
  private callNativeMethod(object: any, method: string, args: any[]) {
    ...
    if (Array.isArray(object)) {
      switch (method) {
        ...
        case "removeAt": {
          ...
          const idx = args[0];
          if (idx < 0 || idx >= object.length) throw new Error("Index out of range");
        ...
        case "map": {
          ...
          return object.map((item: any) => this.callGlowFunction(args[0], [item]));
        }
        case "filter": {
          // filter(func) — return new array with items where func returns true
          return object.filter((item: any) => this.callGlowFunction(args[0], [item]));
        }
      }
    }
      ...
      switch (method) {
        ...
        case "merge": {
          ...
          if (typeof other === "object" && other !== null) {
            for (const k of Object.keys(other)) object[k] = other[k];
          }
          return null;
        }
        case "toArray": {
  ...
  private callGlowFunction(fn: any, args: any[]): any {
    if (!fn || (fn.type !== "Function" && fn.type !== "NativeFunction")) {
      throw new Error("Expected a function");
    }
    if (fn.type === "NativeFunction") return fn.call(args);
  ...
  private evaluate(expression: Expression, environment: Environment): any {
    switch (expression.type) {
      ...
      case "CallExpression": {
        ...
        if (callee.type === "NativeFunction") return callee.call(args);
  ...
  private callFunction(declaration: FunctionStatement, closure: Environment, args: any[]) {
