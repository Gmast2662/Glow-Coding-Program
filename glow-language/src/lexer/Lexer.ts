...
export let keywords: Record<string, TokenType> = {
  var: TokenType.VAR,
  if: TokenType.IF,
  else: TokenType.ELSE,
 ...
 */
export function applyKeywordConfig(configKeywords: Record<string, string>) {
  const roleToToken: Record<string, TokenType> = {
    var: TokenType.VAR,
    if: TokenType.IF,
    else: TokenType.ELSE,
    while: TokenType.WHILE,
