 ...
 * ║    --code "func upper() { ... }"                         ║
 * ║                                                          ║
 * ║  Saves to ../community/ (glow-project/community)         ║
...
const ARCHIVE_DIR = path.join(COMMUNITY_DIR, "archive");
...
function main() {
    ...
    if (missing.length > 0) {
        ...
        console.log(dim("\nUsage:"));
        console.log(dim('  npm run approve -- \\'));
    ...
    if (existing) {
        console.log(info(`Library "${id}" already exists — overwriting.`));
    ...
    // ── Save .js file to community/ ────────────────────────────────────────────
    const jsPath = path.join(COMMUNITY_DIR, `${id}.js`);
    fs.writeFileSync(jsPath, code + "\n", "utf8");
    console.log(ok(`Saved → community/${id}.js`));

    // ── Save metadata for reference ────────────────────────────────────────────
    const metaPath = path.join(COMMUNITY_DIR, `${id}.meta.json`);
    fs.writeFileSync(
        metaPath,
        JSON.stringify({
            author,
    ...
    console.log(dim(`  1. Review community/${id}.js`));
